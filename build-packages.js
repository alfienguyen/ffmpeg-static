...
const PACKAGES = ["ffmpeg-static", "ffprobe-static"];

const copyFileIntoPackage = (pkgName, filename) => {
  const src = pathJoin(process.cwd(), filename);
  const dest = pathJoin(process.cwd(), "packages", pkgName, filename);
  cpSync(src, dest, {
...
const generatePackageJsonForPackages = (pkgName) => {
  ...
  const dest = pathJoin(process.cwd(), "packages", pkgName, "package.json");
